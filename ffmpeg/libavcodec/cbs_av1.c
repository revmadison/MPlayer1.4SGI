...
#define fbs(width, name, subs, ...) \
        xf_args(width, name, current->name, 0, MAX_UINT_BITS(width), subs, __VA_ARGS__)
#define fcs(width, name, range_min, range_max, subs, ...) \
        xf_args(width, name, current->name, range_min, range_max, subs, __VA_ARGS__)
#define flags(name, subs, ...) \
        xf_args(1, name, current->name, 0, 1, subs, __VA_ARGS__)
#define sus(width, name, subs, ...) \
        xsu_args(width, name, current->name, subs, __VA_ARGS__)
...
#define xf_args(width, name, var, range_min, range_max, subs, ...) do { \
        uint32_t value = range_min; \
        CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, \
...
#define xsu(width, name, var, subs) do { \
        ...
        CHECK(ff_cbs_read_signed(ctx, rw, width, #name, \
                                 SUBSCRIPTS_NOARGS(subs), &value, \
                                 MIN_INT_BITS(width), \
                                 MAX_INT_BITS(width))); \
        var = value; \
    } while (0)

#define xsu_args(width, name, var, subs, ...) do { \
        int32_t value = 0; \
        CHECK(ff_cbs_read_signed(ctx, rw, width, #name, \
