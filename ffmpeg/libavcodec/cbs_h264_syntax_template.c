                                      ...
                                      H264RawSEIBufferingPeriod *current)
{
    ...
    if (sps->vui.nal_hrd_parameters_present_flag) {
        for (i = 0; i <= sps->vui.nal_hrd_parameters.cpb_cnt_minus1; i++) {
            ...
            xu_args(length, initial_cpb_removal_delay[SchedSelIdx],
               current->nal.initial_cpb_removal_delay[i],
               1, MAX_UINT_BITS(length), 1, i);
            xu_args(length, initial_cpb_removal_delay_offset[SchedSelIdx],
                                          ...
                                          uint32_t *payload_size)
{
    ...
    *payload_size = i + current->data_length;
    ...
    for (j = 0; j < current->data_length; j++)
        xu_args(8, itu_t_t35_payload_byte[i], current->data[j], 0x00, 0xff, 1, i + j);
                                            ...
                                            uint32_t *payload_size)
{
    ...
    *payload_size = 16 + current->data_length;
    ...
    for (i = 0; i < current->data_length; i++)
        xu_args(8, user_data_payload_byte[i], current->data[i], 0x00, 0xff, 1, i);
    ...
    default:
        {
            ...
            for (i = 0; i < current->payload.other.data_length; i++)
                xu_args(8, payload_byte[i], current->payload.other.data[i], 0, 255, 1, i);
        }
    }
