...
static av_always_inline av_flatten void FUNCC(h264_loop_filter_luma)(uint8_t *p_pix, ptrdiff_t xstride, ptrdiff_t ystride, int inner_iters, int alpha, int beta, int8_t *tc0)
{
    ...
    const int calpha = alpha << (BIT_DEPTH - 8);
    const int cbeta = beta << (BIT_DEPTH - 8);

    const int n2xstride = -2*xstride;
    const int n3xstride = -3*xstride;
    const int p2xstride = 2*xstride;
    ...
    for( i = 0; i < 4; i++ ) {
        ...
        if(tc_orig) {
			...
			for( d = 0; d < inner_iters; d++ ) {
				...
				const int p1 = pix[n2xstride];
				const int p2 = pix[n3xstride];
				const int q0 = pix[0];
				const int q1 = pix[/*1**/xstride];
				const int q2 = pix[p2xstride];

				if( FFABS( p0 - q0 ) < calpha &&
				FFABS( p1 - p0 ) < cbeta &&
				FFABS( q1 - q0 ) < cbeta ) {
					int tc = tc_orig;
					int i_delta;

					if( FFABS( p2 - p0 ) < cbeta ) {
						if(tc_orig)
						pix[n2xstride] = p1 + av_clip( (( p2 + ( ( p0 + q0 + 1 ) >> 1 ) ) >> 1) - p1, -tc_orig, tc_orig );
						tc++;
					}
					if( FFABS( q2 - q0 ) < cbeta ) {
						if(tc_orig)
