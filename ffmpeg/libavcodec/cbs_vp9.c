...
#define SUBSCRIPTS(subs) (subs > 0 ? ((int[subs + 1]){ subs }) : NULL)
#define SUBSCRIPTS_ARGS(subs, ...) (subs > 0 ? ((int[subs + 1]){ subs, __VA_ARGS__ }) : NULL)
...
#define fs(width, name, subs, ...) \
        xf_args(width, name, current->name, subs, __VA_ARGS__)
#define ss(width, name, subs, ...) \
        xs_args(width, name, current->name, subs, __VA_ARGS__)
...
#define xs(width, name, var, subs) do { \
        ...
        CHECK(cbs_vp9_read_s(ctx, rw, width, #name, \
                             SUBSCRIPTS(subs), &value)); \
        var = value; \
    } while (0)

#define xf_args(width, name, var, subs, ...) do { \
        uint32_t value = 0; \
        CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, \
                                   SUBSCRIPTS_ARGS(subs, __VA_ARGS__), \
                                   &value, 0, (1 << width) - 1)); \
        var = value; \
    } while (0)
#define xs_args(width, name, var, subs, ...) do { \
        int32_t value = 0; \
        CHECK(cbs_vp9_read_s(ctx, rw, width, #name, \
                             SUBSCRIPTS_ARGS(subs, __VA_ARGS__), &value)); \
        var = value; \
    } while (0)
...
#define fle(width, name, subs, ...) do { \
        CHECK(cbs_vp9_read_le(ctx, rw, width, #name, \
                              SUBSCRIPTS_ARGS(subs, __VA_ARGS__), &current->name)); \
    } while (0)
...
#define prob(name, subs, ...) do { \
        uint8_t prob_coded; \
        uint8_t prob; \
        xf_args(1, name.prob_coded, prob_coded, subs, __VA_ARGS__); \
        if (prob_coded) \
            xf_args(8, name.prob, prob, subs, __VA_ARGS__); \
        else \
            prob = 255; \
        current->name = prob; \
    } while (0)
...
#define xs_args(width, name, var, subs, ...) do { \
        CHECK(cbs_vp9_write_s(ctx, rw, width, #name, \
                              SUBSCRIPTS_ARGS(subs, __VA_ARGS__), var)); \
    } while (0)
...
#define prob(name, subs, ...) do { \
        xf_args(1, name.prob_coded, current->name != 255, subs, __VA_ARGS__); \
        if (current->name != 255) \
            xf_args(8, name.prob, current->name, subs, __VA_ARGS__); \
    } while (0)
