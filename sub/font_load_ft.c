...
#else
#define WARNING(msg, ...)      mp_msg(MSGT_OSD, MSGL_WARN, msg "\n", __VA_ARGS__)
#endif
...
//    fprintf(stderr, "select unicode charmap: %d\n", error);
	...
	WARNING("Unicode charmap not available for this font. Very bad!", NULL);
	uni_charmap = 0;
	error = FT_Set_Charmap(face, face->charmaps[0]);
	if (error) WARNING("No charmaps! Strange.", NULL);
    }

    /* set size */
    if (FT_IS_SCALABLE(face)) {
	error = FT_Set_Char_Size(face, 0, floatTof266(ppem), 0, 0);
	if (error) WARNING("FT_Set_Char_Size failed.", NULL);
    } else {
	int j = 0;
	...
	if (error) WARNING("FT_Set_Pixel_Sizes failed.", NULL);
    }
